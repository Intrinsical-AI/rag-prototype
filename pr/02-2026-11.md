# PR11: Hardening de consistencia transaccional y invalidación multi-worker

Base: `pr/02-2026-10` (stacked PR).

## Contexto
Durante la auditoría técnica se detectaron dos riesgos de alta criticidad:

1. En `dense/hybrid`, un fallo del embedder podía dejar cambios en SQLite ya comprometidos sin reflejo en el índice vectorial.
2. La invalidación del caché RAG multi-worker escribía un `.tmp` fijo para el reload token, con riesgo de colisión entre procesos.

## Cambios principales

### 1) Atomicidad lógica SQL + índice frente a fallos de embedding
- API:
  - `POST /api/docs`
  - `POST /api/docs/upsert`
- CLI:
  - `rag-upsert-docs`
  - `rag-ingest`

Nuevo flujo:
- Detecta previamente qué `external_id` requieren embedding real (insert o cambio de contenido).
- Pre-computa embeddings **antes** del `upsert` SQL.
- Ejecuta `upsert` SQL.
- Actualiza índice solo para `content_changed`.

Resultado:
- Si el embedder falla, no hay commit parcial en SQL para esos cambios.
- Se mantiene idempotencia (sin embeddings en no-op).

### 2) Hardening de `reset_rag_service` multi-worker
- `app/factory.py` ahora escribe el reload token con `NamedTemporaryFile` único + `fsync` + `os.replace`.
- Evita colisiones del `.tmp` fijo entre workers/procesos.

### 3) Soporte en repositorio SQL para pre-chequeo de cambios
- Nuevo helper: `SqlDocumentStorage.get_existing_doc_states_by_external_id(...)`.
- Permite decidir selectivamente qué documentos necesitan embedding antes de comprometer SQL.

## Tests añadidos
- `tests/unit/app/test_docs_embedding_atomicity.py`
  - Valida que `/api/docs` y `/api/docs/upsert` no persisten en SQL si falla embedder.
- `tests/unit/test_cli_upsert_docs.py`
  - Valida no persistencia en `rag-upsert-docs` ante fallo de embedder.
- `tests/unit/test_cli_ingest.py`
  - Valida no persistencia en `rag-ingest` (dense) ante fallo de embedder.
- `tests/unit/app/test_factory_cache_invalidation.py`
  - Valida que `_write_reload_token` no reutiliza rutas temporales.

## Verificación
- `pytest -q` ✅ (251 passed, cobertura total 85.20%)
- `ruff check src tests` ✅
- `black --check src tests` ✅
- `mypy src` ✅
- `isort --check-only src tests` ❌
  - Quedan fallos preexistentes en archivos no tocados por este PR (`app/middleware.py`, `scripts/*`, `tests/conftest.py`, etc.).

## Commits incluidos
- `5fa6c06` fix: prevent SQL/vector drift when embedding fails
- `fd412d8` fix: harden rag service reload token writes

---

## Auditoría adicional (continuación)
Se añadieron dos hardenings críticos adicionales:

### 4) Serialización de escrituras multi-store en API
Riesgo detectado:
- Dos `upserts` concurrentes sobre el mismo `external_id` podían intercalar SQL+FAISS y dejar vector stale respecto al contenido final en SQL.

Fix:
- Nuevo lock cross-process/thread: `core/services/write_lock.py`.
- Los endpoints mutantes (`/api/docs`, `/api/docs/upsert`, `/api/docs/delete*`, `/api/index/rebuild`) ahora se ejecutan bajo ese lock compartido.

Tests:
- `tests/unit/app/test_multistore_write_lock.py`
  - Simula interleaving forzado y valida consistencia final SQL/vector.

### 5) Validación estricta de parámetros de generación (cost/risk guard)
Riesgo detectado:
- `top_p`, `temperature` y `max_tokens` no estaban acotados en `/api/openrouter/generate`.
- `top_p` en `ask_eval` no tenía validación de rango en schema.

Fix:
- `OpenRouterGenerateRequest` ahora valida:
  - `temperature` en `[0.0, 2.0]`
  - `top_p` en `[0.0, 1.0]`
  - `max_tokens` en `[1, 4096]`
- `AskEvalConfig.top_p` ahora restringido a `[0.0, 1.0]`.
- `validate_rag_config()` también valida `top_p` explícitamente.

Tests:
- `tests/unit/app/test_request_limits.py`
  - Rechazo de sampling params inválidos en OpenRouter.
- `tests/unit/app/test_api_router_more.py`
  - Casos inválidos de `top_p` en `ask_eval`.

## Verificación (actualizada)
- `pytest -q` ✅ (255 passed, cobertura total 85.29%)
- `ruff check src tests` ✅
- `black --check src tests` ✅
- `mypy src` ✅
- `isort --check-only src tests` ❌
  - Persisten fallos preexistentes fuera del scope crítico.

---

## Auditoría integral (esta iteración)
Se identificaron y corrigieron 3 hallazgos críticos adicionales de consistencia/concurrencia:

### 6) `/api/docs` ejecutaba ingestión dos veces (segunda pasada fuera del lock)
Impacto sistémico:
- Doble ejecución de la ruta mutante (coste duplicado y ventana de interleaving sin lock).
- Riesgo de respuestas inconsistentes ante concurrencia (segunda pasada fuera de serialización multi-store).

Fix concreto:
- Eliminada la segunda llamada redundante a `_ingest_sync` en `POST /api/docs`.
- Archivo: `src/local_rag_backend/app/api_router.py`.

Test:
- `tests/unit/app/test_multistore_write_lock.py::test_docs_ingest_executes_single_locked_mutation_pass`
  - Verifica que la ingestión se ejecuta una sola vez y bajo wrapper lockeado.

### 7) `multi_store_write_lock` podía fallar en modo silencioso (fail-open)
Impacto sistémico:
- Si fallaba `flock/msvcrt`, la operación mutante continuaba sin lock cross-process.
- Esto anulaba la garantía de serialización SQL+índice en escenarios degradados.

Fix concreto:
- `write_lock` ahora falla en modo explícito (`RuntimeError`) cuando no puede adquirir lock OS.
- Archivo: `src/local_rag_backend/core/services/write_lock.py`.

Test:
- `tests/unit/core/services/test_write_lock.py::test_exclusive_file_lock_fails_closed_when_os_locking_is_unavailable`
  - Simula falla de `fcntl`/`msvcrt` y valida comportamiento fail-closed.

### 8) Comandos mutantes de CLI no usaban lock multi-store compartido
Impacto sistémico:
- Interleavings entre CLI/API podían dejar drift SQL/vector durante `upsert/delete/rebuild/ingest`.
- Riesgo especialmente alto en operaciones largas (`rag-ingest`).

Fix concreto:
- Nuevo helper `_run_with_multi_store_write_lock(...)` en CLI.
- Aplicado a:
  - `rag-build-index`
  - `rag-rebuild-index`
  - `rag-delete-docs`
  - `rag-delete-external-ids`
  - `rag-upsert-docs`
  - `rag-bootstrap`
  - `rag-ingest` (tramo mutante por archivo).
- Archivo: `src/local_rag_backend/cli.py`.

Tests:
- `tests/unit/test_cli_write_lock.py::test_cli_upsert_docs_runs_under_multi_store_lock`
- `tests/unit/test_cli_write_lock.py::test_cli_delete_docs_runs_under_multi_store_lock`
- `tests/unit/test_cli_write_lock.py::test_cli_ingest_runs_each_file_mutation_under_multi_store_lock`

## Verificación (esta iteración)
- `pytest -q` ✅ (271 passed, cobertura 85.72%)
- `mypy src` ✅
- `black --check src tests` ✅
- `ruff check --no-fix` (scope de archivos tocados) ✅
- `isort --check-only` (scope de archivos tocados) ✅

---

## Auditoría integral (esta iteración - continuación)
Se detectaron y corrigieron 3 riesgos críticos adicionales de consistencia/concurrencia:

### 9) Índice legacy sin manifest podía ser “aceptado” en escrituras incrementales
Impacto sistémico:
- Si existía un índice no vacío previo a PR9 (sin `index_manifest.json`), una escritura incremental podía crear manifest “a posteriori” con la config actual.
- Eso permitía mezclar embeddings históricos de configuración desconocida con embeddings nuevos, ocultando drift semántico y degradando retrieval de forma silenciosa.

Fix concreto:
- `FaissVectorStorage._ensure_manifest()` ahora falla en modo cerrado cuando:
  - falta manifest, y
  - el índice ya contiene vectores/id_map.
- En ese caso exige rebuild explícito (`rag-rebuild-index` / `POST /api/index/rebuild`).
- Archivo: `src/local_rag_backend/infrastructure/persistence/faiss/faiss_.py`.

Test:
- `tests/unit/infrastructure/persistence/faiss/test_vector_storage_manifest_guard.py::test_upsert_fails_closed_when_manifest_is_missing_for_non_empty_legacy_index`

### 10) Invalidación de caché RAG no se ejecutaba en fallos de operaciones mutantes
Impacto sistémico:
- Si una operación mutante fallaba después de modificar parcialmente estado (p.ej. SQL mutado + fallo en sync/rebuild de índice), no se emitía `reset_rag_service()`.
- Riesgo de workers con retriever/corpus stale tras error, empeorando inconsistencia observada por clientes.

Fix concreto:
- API mutante: `reset_rag_service()` se ejecuta en `finally` para `/api/docs`, `/api/docs/upsert`, `/api/docs/delete*`, `/api/index/rebuild`.
- CLI mutante: invalidación best-effort en `finally` para `build-index`, `rebuild-index`, `delete-docs`, `delete-external-ids`, `upsert-docs`, `bootstrap`, `ingest`.
- Archivo principal: `src/local_rag_backend/app/api_router.py`, `src/local_rag_backend/cli.py`.

Tests:
- `tests/unit/app/test_multistore_write_lock.py::test_upsert_failure_still_invalidates_cached_rag_service`
- `tests/unit/test_cli_upsert_docs.py::test_cli_upsert_docs_failure_still_invalidates_cached_rag_service`

### 11) Lock de archivo FAISS en modo fail-open
Impacto sistémico:
- Si `fcntl/msvcrt` fallaba, `FaissIndex` continuaba operaciones sin lock cross-process.
- En escenarios degradados/plataforma atípica, podía haber writes concurrentes y corrupción de `index`/`id_map`.

Fix concreto:
- `_exclusive_file_lock()` de FAISS ahora falla en modo cerrado (`RuntimeError`) cuando no puede adquirir lock OS.
- Archivo: `src/local_rag_backend/infrastructure/persistence/faiss/index.py`.

Test:
- `tests/unit/infrastructure/persistence/faiss/test_faiss_index_lock.py::test_exclusive_file_lock_fails_closed_when_os_locking_is_unavailable`

## Verificación (actualizada)
- `pytest -q` ✅ (275 passed, cobertura 85.83%)
- `ruff check --no-fix src tests` ✅
- `black --check src tests` ✅
- `mypy src` ✅
- `isort --check-only src tests` ✅

Nota técnica adicional:
- Se alineó config de `isort` con `ruff.lint.isort` (`combine_as_imports=true`) para eliminar conflicto de formato entre linters y mantener checks deterministas.

---

## Auditoría integral (esta iteración - continuación)
Se detectaron y corrigieron 2 riesgos críticos adicionales:

### 14) Bypass de protección por bind público al arrancar con override de host en uvicorn
Impacto sistémico:
- La protección previa (`enforce_safe_bind_config`) validaba solo `settings.app_host` al arranque.
- Si el servicio se exponía con override externo de host (p. ej. `uvicorn ... --host 0.0.0.0`) con `API_KEY` vacío, podía aceptar tráfico remoto a endpoints sensibles (`/api/*`, `/metrics`).

Fix concreto:
- `require_api_key()` ahora aplica guard runtime:
  - Si `API_KEY` no está configurado y `PUBLIC_BIND_REQUIRES_API_KEY=true`, rechaza requests no locales (`401`).
  - Mantiene acceso local sin API key para desarrollo.
  - Respeta override explícito (`PUBLIC_BIND_REQUIRES_API_KEY=false`).
- Archivo: `src/local_rag_backend/app/security.py`.

Tests:
- `tests/unit/app/test_api_key_auth.py::test_non_local_requests_require_api_key_when_public_bind_guard_enabled`
- `tests/unit/app/test_api_key_auth.py::test_non_local_requests_can_be_allowed_explicitly`

### 15) Ruta de coordinación cross-process dependía de `DATA_DIR` relativo (riesgo de desalineación entre procesos)
Impacto sistémico:
- El lock multi-store y el token de invalidación usaban `settings.data_dir` directamente.
- Con `SQLITE_URL` absoluto y `DATA_DIR` default/relativo, procesos con distinto `cwd` podían usar rutas de coordinación distintas, perdiendo serialización e invalidación compartida.

Fix concreto:
- Nuevo helper: `Settings.get_coordination_dir()`.
  - Prioriza `DATA_DIR` explícito (no default).
  - Si `DATA_DIR` es default y `SQLITE_URL` es absoluto, usa el directorio padre del DB.
  - Fallback: `DATA_DIR` resuelto.
- `multi_store_write_lock` y `_reload_token_path` ahora usan `get_coordination_dir()`.
- Archivos:
  - `src/local_rag_backend/settings.py`
  - `src/local_rag_backend/core/services/write_lock.py`
  - `src/local_rag_backend/app/factory.py`

Tests:
- `tests/unit/settings/test_settings_validators.py::test_coordination_dir_prefers_explicit_data_dir`
- `tests/unit/settings/test_settings_validators.py::test_coordination_dir_uses_absolute_sqlite_parent_when_data_dir_is_default`
- `tests/unit/core/services/test_write_lock.py::test_multi_store_write_lock_uses_coordination_dir`
- `tests/unit/app/test_factory_cache_invalidation.py::test_reload_token_path_uses_coordination_dir`

## Verificación (actualizada)
- `pytest -q` ✅ (287 passed, cobertura 86.20%)
- `.venv/bin/ruff check src tests` ✅
- `.venv/bin/black --check src tests` ✅
- `.venv/bin/mypy src` ✅

---

## Auditoría integral (esta iteración - hardening adicional)
Se detectaron y corrigieron 2 riesgos críticos adicionales:

### 12) `delete_by_external_ids` / tombstones rompían con `IntegrityError` ante IDs duplicados
Impacto sistémico:
- Entradas con `external_ids` duplicados (incluyendo variantes con whitespace) podían causar `UNIQUE constraint failed` en `document_tombstones`.
- La operación completa se abortaba por transacción (delete + tombstone), dejando fallos 500 no deterministas en endpoints/CLI de borrado.

Fix concreto:
- Normalización centralizada de `external_ids` en repositorio SQL:
  - trim
  - eliminación de vacíos
  - deduplicación preservando orden
- Aplicado en:
  - `get_tombstoned_external_ids(...)`
  - `tombstone_external_ids(...)`
  - `delete_by_external_ids(...)`
- Validación adicional en schema API:
  - `DeleteDocsByExternalIdRequest.external_ids` ahora valida `max_length=512` por elemento y deduplica/normaliza la lista.
- Archivos:
  - `src/local_rag_backend/infrastructure/persistence/sqlalchemy/sql_.py`
  - `src/local_rag_backend/app/schemas.py`

Tests:
- `tests/unit/infrastructure/persistence/sqlalchemy/test_sql_storage.py::test_tombstone_external_ids_deduplicates_request`
- `tests/unit/infrastructure/persistence/sqlalchemy/test_sql_storage.py::test_delete_by_external_ids_accepts_duplicates_without_integrity_error`
- `tests/unit/app/test_docs_delete_by_external_id.py::test_delete_by_external_id_deduplicates_request_values`
- `tests/unit/app/test_request_limits.py::test_delete_by_external_id_rejects_overlong_values`

### 13) Clientes OpenAI/OpenRouter sin timeout explícito (riesgo de bloqueo indefinido)
Impacto sistémico:
- Llamadas de red potencialmente colgadas podían ocupar workers de bloqueo indefinidamente.
- En rutas mutantes dense/hybrid, esto puede prolongar lock multi-store y degradar disponibilidad global.

Fix concreto:
- Nuevo setting `OPENAI_REQUEST_TIMEOUT` (default: `60s`).
- Timeout aplicado en:
  - `OpenAIEmbedder`
  - `OpenAIGenerator`
  - proxy `POST /api/openrouter/generate`
- Fallback compatible: si un cliente/test double no soporta `timeout`, se reintenta sin ese parámetro.
- Archivos:
  - `src/local_rag_backend/settings.py`
  - `src/local_rag_backend/infrastructure/embeddings/openai.py`
  - `src/local_rag_backend/infrastructure/llms/openai_chat.py`
  - `src/local_rag_backend/app/api_router.py`
  - `.env.example`
  - `README.md`

Tests:
- `tests/unit/infrastructure/embeddings/test_openai_embedder.py::test_openai_embedder_passes_configured_timeout`
- `tests/unit/infrastructure/llms/test_openai_generator.py::test_generator_passes_configured_timeout`
- `tests/unit/app/test_api_router_docs_and_eval.py::test_openrouter_generate_success` (assert timeout propagado)

## Verificación (actualizada)
- `pytest -q` ✅ (281 passed, cobertura 86.00%)
- `ruff check src tests` ✅
- `ruff format --check src tests` ✅
- `mypy .` ✅

---

## Auditoría integral (esta iteración - deduplicación estructural crítica)
Se ejecutó un repaso específico de duplicación/código redundante en rutas mutantes API/CLI y persistencia.  
Se corrigieron los siguientes riesgos de drift funcional:

### A) Flujo duplicado de precompute embeddings + sync vectorial en API/CLI
Impacto sistémico:
- Dos implementaciones casi idénticas del mismo flujo crítico podían divergir con fixes futuros.
- Riesgo directo de inconsistencia SQL/FAISS entre rutas mutantes.

Fix concreto:
- Nuevo módulo compartido: `src/local_rag_backend/core/services/dense_upsert.py`
  - `precompute_vectors_for_changed_items(...)`
  - `sync_dense_after_upsert(...)`
- API (`/api/docs`, `/api/docs/upsert`) y CLI (`ingest`, `upsert-docs`) migrados al helper común.
- `sync_dense_after_upsert(...)` permite inyectar `rebuild_fn` para mantener testabilidad/control de fallback.

Tests:
- `tests/unit/core/services/test_dense_upsert.py`
  - cobertura de precompute selectivo (new/changed/rehash-needed)
  - fail-fast por mismatch de cantidad de vectores
  - sync incremental feliz
  - error por vectores faltantes
  - fallback a rebuild ante fallo en upsert

### B) Duplicación de file-lock cross-process
Impacto sistémico:
- Dos implementaciones de locking crítico (POSIX/Windows/fail-closed) aumentaban riesgo de mantenimiento asimétrico.

Fix concreto:
- Nuevo módulo compartido: `src/local_rag_backend/core/services/file_lock.py`
- Reuso desde:
  - `src/local_rag_backend/core/services/write_lock.py`
  - `src/local_rag_backend/infrastructure/persistence/faiss/index.py`

### C) Duplicación de expected_manifest en API/CLI/FAISS
Impacto sistémico:
- Tres copias de configuración esperada podían derivar en drift silencioso del guard de manifest.

Fix concreto:
- Nuevo helper: `expected_manifest_config_from_settings(...)` en
  `src/local_rag_backend/infrastructure/persistence/faiss/manifest.py`
- Reutilizado en API readiness, CLI status y adapter FAISS.

Tests:
- `tests/unit/infrastructure/persistence/faiss/test_manifest.py::test_expected_manifest_config_from_settings_switches_backend`

### D) Duplicación de construcción de cliente OpenAI/OpenRouter con fallback timeout
Impacto sistémico:
- Patrón repetido en varias capas; cualquier cambio de compatibilidad del SDK exigía tocar varios puntos.

Fix concreto:
- Nuevo helper único: `src/local_rag_backend/infrastructure/llms/openai_client.py::create_openai_client(...)`
- Adoptado en:
  - `OpenAIEmbedder`
  - `OpenAIGenerator`
  - endpoint `/api/openrouter/generate`
- Hardening adicional:
  - no envía kwargs opcionales en `None` (mejor compatibilidad con test doubles/SDKs)
  - fallback sólo para TypeError asociado a `timeout`.

Tests:
- `tests/unit/infrastructure/llms/test_openai_client.py`

### E) Código muerto real en CRUD
Impacto sistémico:
- Superficie innecesaria y ruido de mantenimiento.

Fix concreto:
- Eliminada función no utilizada `get_documents(...)` en
  `src/local_rag_backend/infrastructure/persistence/sqlalchemy/crud.py`.

### F) Validación redundante en `ask_eval` (schema vs validación manual)
Impacto sistémico:
- Dos fuentes de verdad para límites numéricos podían responder distinto (422 vs 400).

Fix concreto:
- Límites numéricos centralizados en schema (`AskEvalConfig`):
  - `temperature` `[0.0, 2.0]`
  - `max_tokens` `[1, 4096]`
- `validate_rag_config()` queda orientado a reglas cross-field (modo retrieval + template).

Tests:
- Ajuste de casos esperados a 422 en `tests/unit/app/test_api_router_more.py`.

### G) Métrica de borrado vectorial inexacta (`deleted_index`)
Impacto sistémico:
- Se reportaba tamaño de input aunque no todos los IDs existieran en índice.

Fix concreto:
- `VectorRepoPort.delete(...)` pasa a retornar conteo real (`int`).
- Implementación FAISS retorna conteo real desde `delete_ids(...)`.
- API/CLI usan el valor real retornado.
- `delete_documents_multi_store(...)` alinea respuesta con conteo retornado (con fallback defensivo si adapter legacy retorna `None`).

Tests:
- `tests/unit/core/services/test_maintenance.py::test_delete_documents_uses_real_deleted_index_count`
- Ajustes en `tests/unit/app/test_docs_delete_by_external_id.py` (dummy vector retorna `int`).

### H) Detección duplicada de providers en `/config`
Impacto sistémico:
- Redundancia innecesaria y riesgo de discrepancia con selector principal de providers.

Fix concreto:
- `/api/config` deriva `available_providers` desde `get_available_llm_providers()`.

### I) Documentación adicional incluida en este PR
- Se incluyen los archivos añadidos en `docs/curso/`:
  - `docs/curso/INDICE-1.md`
  - `docs/curso/INDICE-2.md`

## Verificación (final de esta iteración)
- `pytest -q` ✅ (297 passed, cobertura 87.10%)
- `ruff check .` ✅
- `ruff format --check .` ✅
- `mypy .` ✅
- Nota: `make sync` no pudo ejecutarse por restricción de red/DNS hacia `pypi.org`; la validación se ejecutó con entorno ya provisionado vía `uv run --active --no-sync`.
