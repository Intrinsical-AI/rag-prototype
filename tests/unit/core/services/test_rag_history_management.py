...
class TestRagHistoryManagement:
    ...
    def test_history_disabled_via_settings(
    ...
    def test_history_no_cleanup_with_unlimited(
        self, mock_retriever, mock_generator, mock_history_storage
    ):
    ...
    def test_history_disabled_for_empty_results(
        self, mock_retriever, mock_generator, mock_history_storage
    ):
    ...
    def test_history_enabled_for_empty_results(
        self, mock_retriever, mock_generator, mock_history_storage
    ):
        ...
        with patch("local_rag_backend.core.services.rag.settings", enabled_settings):
            ...
            mock_history_storage.save.assert_called_once_with(
                "test question", "No documents are available to answer your question.", []
            )

    @pytest.mark.parametrize(
        "max_entries,should_cleanup",
        [
            (0, False),  # Unlimited
    ...
    def test_history_cleanup_various_limits(
        self, mock_retriever, mock_generator, mock_history_storage, max_entries, should_cleanup
    ):
    ...
    def test_history_environment_variable_configuration(
        self, mock_retriever, mock_generator, mock_history_storage
    ):
        """Test history configuration via environment variables."""
        with patch.dict("os.environ", {"ENABLE_HISTORY": "false", "MAX_HISTORY_ENTRIES": "500"}):
    ...
    def test_history_multiple_interactions_cleanup(
        self, mock_retriever, mock_generator, mock_history_storage
    ):
    ...
    def test_history_performance_impact_minimal(
        self, mock_retriever, mock_generator, mock_history_storage
    ):
