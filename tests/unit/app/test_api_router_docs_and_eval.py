...
def test_post_docs_sparse_and_list(in_memory_sqlite, monkeypatch):
    monkeypatch.setattr(settings, "retrieval_mode", "sparse", raising=False)
    payload = {"texts": ["  First  ", "", "Second"]}
    r = client.post("/api/docs", json=payload)
    ...
    # Now list
    r2 = client.get("/api/docs", params={"limit": 10, "offset": 0})
...
def test_post_docs_sparse_various_inputs(in_memory_sqlite, monkeypatch, texts, expected):
    monkeypatch.setattr(settings, "retrieval_mode", "sparse", raising=False)
    r = client.post("/api/docs", json={"texts": texts})
    assert r.status_code == 200
    data = r.json()
    assert data["count"] == expected


def test_post_docs_dense_uses_etl(in_memory_sqlite, monkeypatch):
...
def test_ask_eval_sparse_success(in_memory_sqlite, monkeypatch):
...
def test_ready_retrieval_index_present(tmp_path, monkeypatch):
