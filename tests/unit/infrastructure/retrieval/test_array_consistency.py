...
class TestArrayConsistencyFixes:
    ...
    @pytest.mark.parametrize(
        "missing_doc_ids,expected_count",
        [
    ...
    def test_dense_faiss_retriever_missing_documents(
        ...
        mock_faiss_index.similar.return_value = [(1, 0.9), (2, 0.8), (3, 0.7)]
        ...
        assert len(docs) == len(scores), (
            f"Array length mismatch: {len(docs)} docs vs {len(scores)} scores"
        )
        ...
        assert returned_ids == expected_ids, (
            f"Document IDs mismatch: {returned_ids} vs {expected_ids}"
        )
