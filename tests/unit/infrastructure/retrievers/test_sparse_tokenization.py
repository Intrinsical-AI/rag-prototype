...
class Repo:
    ...
    def get(self, ids): return [d for d in self.docs if d.id in ids]
...
def test_sparse_preprocess_and_norm(monkeypatch):
    ...
    try:
        ...
        class DummyBM25:
            ...
            def get_scores(self, q): return [1.0 if "hola" in q else 0.2, 0.0]
        monkeypatch.setattr("local_rag_backend.infrastructure.retrieval.sparse_bm25.BM25Okapi", DummyBM25, raising=False)

    retr = SparseBM25Retriever(
        documents=["<b>Hola</b> mundo", "Adios..."],
        doc_ids=[1,2],
        doc_repo=Repo()
    )
