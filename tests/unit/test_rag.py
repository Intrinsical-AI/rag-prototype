...
)  # Alias para evitar confusión con un posible Document de Pydantic
...
class DummyRetriever(RetrieverPort):  # Implementar el Puerto
    def __init__(self, doc_ids: list[int] = None, scores: list[float] = None):
        ...
        self.scores_to_return = (
            scores if scores is not None else [0.9] * len(self.doc_ids_to_return)
        )
        if len(self.doc_ids_to_return) != len(self.scores_to_return):
            raise ValueError(
                "doc_ids y scores deben tener la misma longitud en DummyRetriever"
            )
...
def test_rag_service_ask_no_documents_found(db_session: Session):
    ...
    assert (
        generator_spy.called_with_contexts == []
    )  # Verifica que el contexto estaba vacío
