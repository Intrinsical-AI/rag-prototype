...
def resolve_loader_options(
    *, settings_obj: Settings, csv_delimiter: str, csv_has_header: bool | None
) -> tuple[str | None, bool]:
    delimiter_opt = None if csv_delimiter.strip().lower() == "auto" else csv_delimiter
    has_header = settings_obj.csv_has_header if csv_has_header is None else bool(csv_has_header)
