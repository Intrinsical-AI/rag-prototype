"""Internal provider/cache construction shared with the composition layer."""
...
def build_dense_embedder_from_settings(
    *,
    ...
    """Build the existing provider plus content-addressed cache stack."""
    resolved_openai_factory = openai_embedder_factory or (
        lambda: _build_default_openai_embedder(settings_obj)
    )
    resolved_st_factory = st_embedder_factory or (
        lambda model_name: _build_default_st_embedder(model_name, settings_obj)
    )
